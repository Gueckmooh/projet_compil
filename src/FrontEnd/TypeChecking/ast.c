...
ptree build_unary(enum code code, ptree t){
    ptree new_t = (ptree) malloc(sizeof(tree));
    new_t->code = code;
    new_t->params.t = t;
    return new_t;
}
 ...
 */
    assert(original_fd);
    pfundef fd_cpy = (pfundef) malloc(sizeof(struct fundef));
...
ptree free_ast(ptree t) {
    ...
    switch (t->code) {
        ...
        case T_ARRAY :
        case T_GET :
            free_ast(t->params.tbinary.t2) ;
            free_ast(t->params.tbinary.t1) ;
            break ;
                        // case : ternary tree
        case T_IF :
        case T_PUT :
            free_ast(t->params.tternary.t1) ;
            free_ast(t->params.tternary.t2) ;
            free_ast(t->params.tternary.t3) ;
            break ;

                        // other cases
        case T_LET :
            free_ast(t->params.tlet.t1) ;
            free_ast(t->params.tlet.t2) ;
            free(t->params.tlet.t->params.var) ;
            free(t->params.tlet.t) ;
